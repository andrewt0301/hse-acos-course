...
1. System calls / system utilities / bash.
   * __TODO__
...
Write a function that takes two arguments: a pointer to an array (`a0`) and the length of the array (`a0`).
The function computes the distance between the smallest and the largest element in the array and returns it in (`a0`).
Be careful about callee-saved registers: (1) do not modify them at all or (2) save them to the stack and then restore.  
