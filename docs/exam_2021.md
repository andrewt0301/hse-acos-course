...
1. Inter-process communications: signals.
   * Give a definition of a signal.
   * What signals to you know (name and purpose)?
   * How to send a signal to a process (system call and utility tool)?
   * How to set up a custom handler for a signal? It is possible to do for all signals?
